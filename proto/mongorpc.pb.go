// Copyright 2021 MongoRPC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.26.0
// 	protoc        v3.17.3
// source: mongorpc.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Null is a special type that is used to represent a null value in a MongoDB
type NullValue int32

const (
	NullValue_NULL_VALUE NullValue = 0
)

// Enum value maps for NullValue.
var (
	NullValue_name = map[int32]string{
		0: "NULL_VALUE",
	}
	NullValue_value = map[string]int32{
		"NULL_VALUE": 0,
	}
)

func (x NullValue) Enum() *NullValue {
	p := new(NullValue)
	*p = x
	return p
}

func (x NullValue) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (NullValue) Descriptor() protoreflect.EnumDescriptor {
	return file_mongorpc_proto_enumTypes[0].Descriptor()
}

func (NullValue) Type() protoreflect.EnumType {
	return &file_mongorpc_proto_enumTypes[0]
}

func (x NullValue) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use NullValue.Descriptor instead.
func (NullValue) EnumDescriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{0}
}

// Array is a special type that is used to represent an array value in a MongoDB
type ArrayValue struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Values []*Value `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty"`
}

func (x *ArrayValue) Reset() {
	*x = ArrayValue{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ArrayValue) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ArrayValue) ProtoMessage() {}

func (x *ArrayValue) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ArrayValue.ProtoReflect.Descriptor instead.
func (*ArrayValue) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{0}
}

func (x *ArrayValue) GetValues() []*Value {
	if x != nil {
		return x.Values
	}
	return nil
}

// Map is a special type that is used to represent a map value in a MongoDB
type MapValue struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Fields map[string]*Value `protobuf:"bytes,1,rep,name=fields,proto3" json:"fields,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
}

func (x *MapValue) Reset() {
	*x = MapValue{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *MapValue) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MapValue) ProtoMessage() {}

func (x *MapValue) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MapValue.ProtoReflect.Descriptor instead.
func (*MapValue) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{1}
}

func (x *MapValue) GetFields() map[string]*Value {
	if x != nil {
		return x.Fields
	}
	return nil
}

// Timestamp is a special type that is used to represent a timestamp value in a MongoDB
type Timestamp struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Seconds int64 `protobuf:"varint,1,opt,name=seconds,proto3" json:"seconds,omitempty"`
	Nanos   int32 `protobuf:"varint,2,opt,name=nanos,proto3" json:"nanos,omitempty"`
}

func (x *Timestamp) Reset() {
	*x = Timestamp{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Timestamp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Timestamp) ProtoMessage() {}

func (x *Timestamp) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Timestamp.ProtoReflect.Descriptor instead.
func (*Timestamp) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{2}
}

func (x *Timestamp) GetSeconds() int64 {
	if x != nil {
		return x.Seconds
	}
	return 0
}

func (x *Timestamp) GetNanos() int32 {
	if x != nil {
		return x.Nanos
	}
	return 0
}

type ObjectID struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
}

func (x *ObjectID) Reset() {
	*x = ObjectID{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ObjectID) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ObjectID) ProtoMessage() {}

func (x *ObjectID) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ObjectID.ProtoReflect.Descriptor instead.
func (*ObjectID) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{3}
}

func (x *ObjectID) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

// Value is a special type that is used to represent a value in a MongoDB
type Value struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Types that are assignable to Type:
	//	*Value_DoubleValue
	//	*Value_StringValue
	//	*Value_MapValue
	//	*Value_ArrayValue
	//	*Value_BytesValue
	//	*Value_BoolValue
	//	*Value_DateValue
	//	*Value_NullValue
	//	*Value_IntegerValue
	//	*Value_ObjectIdValue
	Type isValue_Type `protobuf_oneof:"type"`
}

func (x *Value) Reset() {
	*x = Value{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Value) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Value) ProtoMessage() {}

func (x *Value) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Value.ProtoReflect.Descriptor instead.
func (*Value) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{4}
}

func (m *Value) GetType() isValue_Type {
	if m != nil {
		return m.Type
	}
	return nil
}

func (x *Value) GetDoubleValue() float64 {
	if x, ok := x.GetType().(*Value_DoubleValue); ok {
		return x.DoubleValue
	}
	return 0
}

func (x *Value) GetStringValue() string {
	if x, ok := x.GetType().(*Value_StringValue); ok {
		return x.StringValue
	}
	return ""
}

func (x *Value) GetMapValue() *MapValue {
	if x, ok := x.GetType().(*Value_MapValue); ok {
		return x.MapValue
	}
	return nil
}

func (x *Value) GetArrayValue() *ArrayValue {
	if x, ok := x.GetType().(*Value_ArrayValue); ok {
		return x.ArrayValue
	}
	return nil
}

func (x *Value) GetBytesValue() []byte {
	if x, ok := x.GetType().(*Value_BytesValue); ok {
		return x.BytesValue
	}
	return nil
}

func (x *Value) GetBoolValue() bool {
	if x, ok := x.GetType().(*Value_BoolValue); ok {
		return x.BoolValue
	}
	return false
}

func (x *Value) GetDateValue() *Timestamp {
	if x, ok := x.GetType().(*Value_DateValue); ok {
		return x.DateValue
	}
	return nil
}

func (x *Value) GetNullValue() NullValue {
	if x, ok := x.GetType().(*Value_NullValue); ok {
		return x.NullValue
	}
	return NullValue_NULL_VALUE
}

func (x *Value) GetIntegerValue() int64 {
	if x, ok := x.GetType().(*Value_IntegerValue); ok {
		return x.IntegerValue
	}
	return 0
}

func (x *Value) GetObjectIdValue() *ObjectID {
	if x, ok := x.GetType().(*Value_ObjectIdValue); ok {
		return x.ObjectIdValue
	}
	return nil
}

type isValue_Type interface {
	isValue_Type()
}

type Value_DoubleValue struct {
	// double represents a double value in a MongoDB
	DoubleValue float64 `protobuf:"fixed64,1,opt,name=double_value,json=doubleValue,proto3,oneof"`
}

type Value_StringValue struct {
	// string represents a string value in a MongoDB
	StringValue string `protobuf:"bytes,2,opt,name=string_value,json=stringValue,proto3,oneof"`
}

type Value_MapValue struct {
	// Map represents a map value in a MongoDB
	MapValue *MapValue `protobuf:"bytes,3,opt,name=map_value,json=mapValue,proto3,oneof"`
}

type Value_ArrayValue struct {
	// Array represents an array value in a MongoDB
	ArrayValue *ArrayValue `protobuf:"bytes,4,opt,name=array_value,json=arrayValue,proto3,oneof"`
}

type Value_BytesValue struct {
	// Bytes represents a bytes value in a MongoDB
	BytesValue []byte `protobuf:"bytes,5,opt,name=bytes_value,json=bytesValue,proto3,oneof"`
}

type Value_BoolValue struct {
	// Boolean represents a boolean value in a MongoDB
	BoolValue bool `protobuf:"varint,8,opt,name=bool_value,json=boolValue,proto3,oneof"`
}

type Value_DateValue struct {
	// Timestamp represents a timestamp value in a MongoDB
	DateValue *Timestamp `protobuf:"bytes,9,opt,name=date_value,json=dateValue,proto3,oneof"`
}

type Value_NullValue struct {
	// Null represents a null value in a MongoDB
	NullValue NullValue `protobuf:"varint,10,opt,name=null_value,json=nullValue,proto3,enum=mongorpc.NullValue,oneof"`
}

type Value_IntegerValue struct {
	// integer represents an integer value in a MongoDB
	IntegerValue int64 `protobuf:"varint,23,opt,name=integer_value,json=integerValue,proto3,oneof"`
}

type Value_ObjectIdValue struct {
	//undefined = 6;
	ObjectIdValue *ObjectID `protobuf:"bytes,7,opt,name=object_id_value,json=objectIdValue,proto3,oneof"`
}

func (*Value_DoubleValue) isValue_Type() {}

func (*Value_StringValue) isValue_Type() {}

func (*Value_MapValue) isValue_Type() {}

func (*Value_ArrayValue) isValue_Type() {}

func (*Value_BytesValue) isValue_Type() {}

func (*Value_BoolValue) isValue_Type() {}

func (*Value_DateValue) isValue_Type() {}

func (*Value_NullValue) isValue_Type() {}

func (*Value_IntegerValue) isValue_Type() {}

func (*Value_ObjectIdValue) isValue_Type() {}

// Set the Sort Order
type Sort struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field     string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Ascending bool   `protobuf:"varint,2,opt,name=ascending,proto3" json:"ascending,omitempty"`
}

func (x *Sort) Reset() {
	*x = Sort{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Sort) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Sort) ProtoMessage() {}

func (x *Sort) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Sort.ProtoReflect.Descriptor instead.
func (*Sort) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{5}
}

func (x *Sort) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Sort) GetAscending() bool {
	if x != nil {
		return x.Ascending
	}
	return false
}

// Filter is a special type that is used to represent a filter in a MongoDB
type Filter struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *Filter) Reset() {
	*x = Filter{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter) ProtoMessage() {}

func (x *Filter) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter.ProtoReflect.Descriptor instead.
func (*Filter) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6}
}

// ListCollectionsRequest is a special type that is used to represent a list collections request in a MongoDB
type ListCollectionsRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Database name
	Database string `protobuf:"bytes,1,opt,name=database,proto3" json:"database,omitempty"`
}

func (x *ListCollectionsRequest) Reset() {
	*x = ListCollectionsRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListCollectionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListCollectionsRequest) ProtoMessage() {}

func (x *ListCollectionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListCollectionsRequest.ProtoReflect.Descriptor instead.
func (*ListCollectionsRequest) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{7}
}

func (x *ListCollectionsRequest) GetDatabase() string {
	if x != nil {
		return x.Database
	}
	return ""
}

// ListCollectionsResponse is a special type that is used to represent a list collections response in a MongoDB
type ListCollectionsResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Array of collection names
	Collections *ArrayValue `protobuf:"bytes,1,opt,name=collections,proto3" json:"collections,omitempty"`
}

func (x *ListCollectionsResponse) Reset() {
	*x = ListCollectionsResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListCollectionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListCollectionsResponse) ProtoMessage() {}

func (x *ListCollectionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListCollectionsResponse.ProtoReflect.Descriptor instead.
func (*ListCollectionsResponse) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{8}
}

func (x *ListCollectionsResponse) GetCollections() *ArrayValue {
	if x != nil {
		return x.Collections
	}
	return nil
}

// GetCollectionRequest is a special type that is used to represent a get collection request in a MongoDB
type GetDocumentRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// The database to query
	Database string `protobuf:"bytes,1,opt,name=database,proto3" json:"database,omitempty"`
	// The collection to query
	Collection string `protobuf:"bytes,2,opt,name=collection,proto3" json:"collection,omitempty"`
	// The document to query
	DocumentId string `protobuf:"bytes,3,opt,name=document_id,json=documentId,proto3" json:"document_id,omitempty"`
}

func (x *GetDocumentRequest) Reset() {
	*x = GetDocumentRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentRequest) ProtoMessage() {}

func (x *GetDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentRequest.ProtoReflect.Descriptor instead.
func (*GetDocumentRequest) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{9}
}

func (x *GetDocumentRequest) GetDatabase() string {
	if x != nil {
		return x.Database
	}
	return ""
}

func (x *GetDocumentRequest) GetCollection() string {
	if x != nil {
		return x.Collection
	}
	return ""
}

func (x *GetDocumentRequest) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

// GetDocumentResponse is a special type that is used to represent a get document response in a MongoDB
type GetDocumentResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Document Value
	Document *Value `protobuf:"bytes,1,opt,name=document,proto3" json:"document,omitempty"`
}

func (x *GetDocumentResponse) Reset() {
	*x = GetDocumentResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *GetDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDocumentResponse) ProtoMessage() {}

func (x *GetDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDocumentResponse.ProtoReflect.Descriptor instead.
func (*GetDocumentResponse) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{10}
}

func (x *GetDocumentResponse) GetDocument() *Value {
	if x != nil {
		return x.Document
	}
	return nil
}

// ListDocumentsRequest is a special type that is used to represent a list documents request in a MongoDB
type ListDocumentsRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// The database to query
	Database string `protobuf:"bytes,1,opt,name=database,proto3" json:"database,omitempty"`
	// The collection to query
	Collection string `protobuf:"bytes,2,opt,name=collection,proto3" json:"collection,omitempty"`
	// Limit the number of documents returned
	Limit int32 `protobuf:"varint,3,opt,name=limit,proto3" json:"limit,omitempty"`
	// Skip the first n documents
	Skip int32 `protobuf:"varint,4,opt,name=skip,proto3" json:"skip,omitempty"`
	// Filter to apply to the query
	Filter []*Filter `protobuf:"bytes,5,rep,name=filter,proto3" json:"filter,omitempty"`
	// Sort to apply to the query
	Sort []*Sort `protobuf:"bytes,6,rep,name=sort,proto3" json:"sort,omitempty"`
}

func (x *ListDocumentsRequest) Reset() {
	*x = ListDocumentsRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[11]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListDocumentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDocumentsRequest) ProtoMessage() {}

func (x *ListDocumentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[11]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDocumentsRequest.ProtoReflect.Descriptor instead.
func (*ListDocumentsRequest) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{11}
}

func (x *ListDocumentsRequest) GetDatabase() string {
	if x != nil {
		return x.Database
	}
	return ""
}

func (x *ListDocumentsRequest) GetCollection() string {
	if x != nil {
		return x.Collection
	}
	return ""
}

func (x *ListDocumentsRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListDocumentsRequest) GetSkip() int32 {
	if x != nil {
		return x.Skip
	}
	return 0
}

func (x *ListDocumentsRequest) GetFilter() []*Filter {
	if x != nil {
		return x.Filter
	}
	return nil
}

func (x *ListDocumentsRequest) GetSort() []*Sort {
	if x != nil {
		return x.Sort
	}
	return nil
}

// ListDocumentsResponse is a special type that is used to represent a list documents response in a MongoDB
type ListDocumentsResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Array of documents
	Documents *ArrayValue `protobuf:"bytes,1,opt,name=documents,proto3" json:"documents,omitempty"`
}

func (x *ListDocumentsResponse) Reset() {
	*x = ListDocumentsResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[12]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListDocumentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDocumentsResponse) ProtoMessage() {}

func (x *ListDocumentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[12]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDocumentsResponse.ProtoReflect.Descriptor instead.
func (*ListDocumentsResponse) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{12}
}

func (x *ListDocumentsResponse) GetDocuments() *ArrayValue {
	if x != nil {
		return x.Documents
	}
	return nil
}

// InsertDocumentRequest is a special type that is used to represent a insert document request in a MongoDB
type CreateDocumentRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// The database to insert the document into
	Database string `protobuf:"bytes,1,opt,name=database,proto3" json:"database,omitempty"`
	// The collection to insert the document into
	Collection string `protobuf:"bytes,2,opt,name=collection,proto3" json:"collection,omitempty"`
	// The document to insert
	Document *Value `protobuf:"bytes,3,opt,name=document,proto3" json:"document,omitempty"`
}

func (x *CreateDocumentRequest) Reset() {
	*x = CreateDocumentRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[13]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateDocumentRequest) ProtoMessage() {}

func (x *CreateDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[13]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateDocumentRequest.ProtoReflect.Descriptor instead.
func (*CreateDocumentRequest) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{13}
}

func (x *CreateDocumentRequest) GetDatabase() string {
	if x != nil {
		return x.Database
	}
	return ""
}

func (x *CreateDocumentRequest) GetCollection() string {
	if x != nil {
		return x.Collection
	}
	return ""
}

func (x *CreateDocumentRequest) GetDocument() *Value {
	if x != nil {
		return x.Document
	}
	return nil
}

// InsertDocumentResponse is a special type that is used to represent a insert document response in a MongoDB
type CreateDocumentResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// document id
	DocumentId string `protobuf:"bytes,1,opt,name=document_id,json=documentId,proto3" json:"document_id,omitempty"`
}

func (x *CreateDocumentResponse) Reset() {
	*x = CreateDocumentResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[14]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateDocumentResponse) ProtoMessage() {}

func (x *CreateDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[14]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateDocumentResponse.ProtoReflect.Descriptor instead.
func (*CreateDocumentResponse) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{14}
}

func (x *CreateDocumentResponse) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

// UpdateDocumentRequest is a special type that is used to represent a update document request in a MongoDB
type UpdateDocumentRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// The database to use
	Database string `protobuf:"bytes,1,opt,name=database,proto3" json:"database,omitempty"`
	// The collection to use
	Collection string `protobuf:"bytes,2,opt,name=collection,proto3" json:"collection,omitempty"`
	// The document to update
	DocumentId string `protobuf:"bytes,3,opt,name=document_id,json=documentId,proto3" json:"document_id,omitempty"`
	// Document Values
	Document *Value `protobuf:"bytes,4,opt,name=document,proto3" json:"document,omitempty"`
}

func (x *UpdateDocumentRequest) Reset() {
	*x = UpdateDocumentRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[15]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateDocumentRequest) ProtoMessage() {}

func (x *UpdateDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[15]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateDocumentRequest.ProtoReflect.Descriptor instead.
func (*UpdateDocumentRequest) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{15}
}

func (x *UpdateDocumentRequest) GetDatabase() string {
	if x != nil {
		return x.Database
	}
	return ""
}

func (x *UpdateDocumentRequest) GetCollection() string {
	if x != nil {
		return x.Collection
	}
	return ""
}

func (x *UpdateDocumentRequest) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

func (x *UpdateDocumentRequest) GetDocument() *Value {
	if x != nil {
		return x.Document
	}
	return nil
}

// UpdateDocumentResponse is a special type that is used to represent a update document response in a MongoDB
type UpdateDocumentResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	MatchedCount  int64  `protobuf:"varint,1,opt,name=matched_count,json=matchedCount,proto3" json:"matched_count,omitempty"`
	ModifiedCount int64  `protobuf:"varint,2,opt,name=modified_count,json=modifiedCount,proto3" json:"modified_count,omitempty"`
	UpsertedCount int64  `protobuf:"varint,3,opt,name=upserted_count,json=upsertedCount,proto3" json:"upserted_count,omitempty"`
	UpsertedId    *Value `protobuf:"bytes,4,opt,name=upserted_id,json=upsertedId,proto3" json:"upserted_id,omitempty"`
}

func (x *UpdateDocumentResponse) Reset() {
	*x = UpdateDocumentResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[16]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateDocumentResponse) ProtoMessage() {}

func (x *UpdateDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[16]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateDocumentResponse.ProtoReflect.Descriptor instead.
func (*UpdateDocumentResponse) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{16}
}

func (x *UpdateDocumentResponse) GetMatchedCount() int64 {
	if x != nil {
		return x.MatchedCount
	}
	return 0
}

func (x *UpdateDocumentResponse) GetModifiedCount() int64 {
	if x != nil {
		return x.ModifiedCount
	}
	return 0
}

func (x *UpdateDocumentResponse) GetUpsertedCount() int64 {
	if x != nil {
		return x.UpsertedCount
	}
	return 0
}

func (x *UpdateDocumentResponse) GetUpsertedId() *Value {
	if x != nil {
		return x.UpsertedId
	}
	return nil
}

// DeleteDocumentRequest is a special type that is used to represent a delete document request in a MongoDB
type DeleteDocumentRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// database name
	Database string `protobuf:"bytes,1,opt,name=database,proto3" json:"database,omitempty"`
	// collection name
	Collection string `protobuf:"bytes,2,opt,name=collection,proto3" json:"collection,omitempty"`
	// document id
	DocumentId string `protobuf:"bytes,3,opt,name=document_id,json=documentId,proto3" json:"document_id,omitempty"`
}

func (x *DeleteDocumentRequest) Reset() {
	*x = DeleteDocumentRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[17]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteDocumentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteDocumentRequest) ProtoMessage() {}

func (x *DeleteDocumentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[17]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteDocumentRequest.ProtoReflect.Descriptor instead.
func (*DeleteDocumentRequest) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{17}
}

func (x *DeleteDocumentRequest) GetDatabase() string {
	if x != nil {
		return x.Database
	}
	return ""
}

func (x *DeleteDocumentRequest) GetCollection() string {
	if x != nil {
		return x.Collection
	}
	return ""
}

func (x *DeleteDocumentRequest) GetDocumentId() string {
	if x != nil {
		return x.DocumentId
	}
	return ""
}

// DeleteDocumentResponse is a special type that is used to represent a delete document response in a MongoDB
type DeleteDocumentResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// Deleted Document Count
	DeletedCount int64 `protobuf:"varint,1,opt,name=deleted_count,json=deletedCount,proto3" json:"deleted_count,omitempty"`
}

func (x *DeleteDocumentResponse) Reset() {
	*x = DeleteDocumentResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[18]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteDocumentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteDocumentResponse) ProtoMessage() {}

func (x *DeleteDocumentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[18]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteDocumentResponse.ProtoReflect.Descriptor instead.
func (*DeleteDocumentResponse) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{18}
}

func (x *DeleteDocumentResponse) GetDeletedCount() int64 {
	if x != nil {
		return x.DeletedCount
	}
	return 0
}

// The key of the filter
type Filter_Operator struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// one of operator
	//
	// Types that are assignable to Operator:
	//	*Filter_Operator_Equal_
	//	*Filter_Operator_NotEqual_
	//	*Filter_Operator_Less_
	//	*Filter_Operator_LessEqual_
	//	*Filter_Operator_Greater_
	//	*Filter_Operator_GreaterEqual_
	//	*Filter_Operator_In_
	//	*Filter_Operator_NotIn_
	//	*Filter_Operator_Exists_
	//	*Filter_Operator_NotExists_
	Operator isFilter_Operator_Operator `protobuf_oneof:"operator"`
}

func (x *Filter_Operator) Reset() {
	*x = Filter_Operator{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[20]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator) ProtoMessage() {}

func (x *Filter_Operator) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[20]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator.ProtoReflect.Descriptor instead.
func (*Filter_Operator) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0}
}

func (m *Filter_Operator) GetOperator() isFilter_Operator_Operator {
	if m != nil {
		return m.Operator
	}
	return nil
}

func (x *Filter_Operator) GetEqual() *Filter_Operator_Equal {
	if x, ok := x.GetOperator().(*Filter_Operator_Equal_); ok {
		return x.Equal
	}
	return nil
}

func (x *Filter_Operator) GetNotEqual() *Filter_Operator_NotEqual {
	if x, ok := x.GetOperator().(*Filter_Operator_NotEqual_); ok {
		return x.NotEqual
	}
	return nil
}

func (x *Filter_Operator) GetLess() *Filter_Operator_Less {
	if x, ok := x.GetOperator().(*Filter_Operator_Less_); ok {
		return x.Less
	}
	return nil
}

func (x *Filter_Operator) GetLessEqual() *Filter_Operator_LessEqual {
	if x, ok := x.GetOperator().(*Filter_Operator_LessEqual_); ok {
		return x.LessEqual
	}
	return nil
}

func (x *Filter_Operator) GetGreater() *Filter_Operator_Greater {
	if x, ok := x.GetOperator().(*Filter_Operator_Greater_); ok {
		return x.Greater
	}
	return nil
}

func (x *Filter_Operator) GetGreaterEqual() *Filter_Operator_GreaterEqual {
	if x, ok := x.GetOperator().(*Filter_Operator_GreaterEqual_); ok {
		return x.GreaterEqual
	}
	return nil
}

func (x *Filter_Operator) GetIn() *Filter_Operator_In {
	if x, ok := x.GetOperator().(*Filter_Operator_In_); ok {
		return x.In
	}
	return nil
}

func (x *Filter_Operator) GetNotIn() *Filter_Operator_NotIn {
	if x, ok := x.GetOperator().(*Filter_Operator_NotIn_); ok {
		return x.NotIn
	}
	return nil
}

func (x *Filter_Operator) GetExists() *Filter_Operator_Exists {
	if x, ok := x.GetOperator().(*Filter_Operator_Exists_); ok {
		return x.Exists
	}
	return nil
}

func (x *Filter_Operator) GetNotExists() *Filter_Operator_NotExists {
	if x, ok := x.GetOperator().(*Filter_Operator_NotExists_); ok {
		return x.NotExists
	}
	return nil
}

type isFilter_Operator_Operator interface {
	isFilter_Operator_Operator()
}

type Filter_Operator_Equal_ struct {
	// Equal represents an equal operator in a MongoDB
	Equal *Filter_Operator_Equal `protobuf:"bytes,1,opt,name=equal,proto3,oneof"`
}

type Filter_Operator_NotEqual_ struct {
	// NotEqual represents a not equal operator in a MongoDB
	NotEqual *Filter_Operator_NotEqual `protobuf:"bytes,2,opt,name=not_equal,json=notEqual,proto3,oneof"`
}

type Filter_Operator_Less_ struct {
	// Less represents a less operator in a MongoDB
	Less *Filter_Operator_Less `protobuf:"bytes,3,opt,name=less,proto3,oneof"`
}

type Filter_Operator_LessEqual_ struct {
	// LessEqual represents a less equal operator in a MongoDB
	LessEqual *Filter_Operator_LessEqual `protobuf:"bytes,4,opt,name=less_equal,json=lessEqual,proto3,oneof"`
}

type Filter_Operator_Greater_ struct {
	// Greater represents a greater operator in a MongoDB
	Greater *Filter_Operator_Greater `protobuf:"bytes,5,opt,name=greater,proto3,oneof"`
}

type Filter_Operator_GreaterEqual_ struct {
	// GreaterEqual represents a greater equal operator in a MongoDB
	GreaterEqual *Filter_Operator_GreaterEqual `protobuf:"bytes,6,opt,name=greater_equal,json=greaterEqual,proto3,oneof"`
}

type Filter_Operator_In_ struct {
	// In represents an in operator in a MongoDB
	In *Filter_Operator_In `protobuf:"bytes,7,opt,name=in,proto3,oneof"`
}

type Filter_Operator_NotIn_ struct {
	// NotIn represents a not in operator in a MongoDB
	NotIn *Filter_Operator_NotIn `protobuf:"bytes,8,opt,name=not_in,json=notIn,proto3,oneof"`
}

type Filter_Operator_Exists_ struct {
	// Exists represents an exists operator in a MongoDB
	Exists *Filter_Operator_Exists `protobuf:"bytes,9,opt,name=exists,proto3,oneof"`
}

type Filter_Operator_NotExists_ struct {
	// NotExists represents a not exists operator in a MongoDB
	NotExists *Filter_Operator_NotExists `protobuf:"bytes,10,opt,name=not_exists,json=notExists,proto3,oneof"`
}

func (*Filter_Operator_Equal_) isFilter_Operator_Operator() {}

func (*Filter_Operator_NotEqual_) isFilter_Operator_Operator() {}

func (*Filter_Operator_Less_) isFilter_Operator_Operator() {}

func (*Filter_Operator_LessEqual_) isFilter_Operator_Operator() {}

func (*Filter_Operator_Greater_) isFilter_Operator_Operator() {}

func (*Filter_Operator_GreaterEqual_) isFilter_Operator_Operator() {}

func (*Filter_Operator_In_) isFilter_Operator_Operator() {}

func (*Filter_Operator_NotIn_) isFilter_Operator_Operator() {}

func (*Filter_Operator_Exists_) isFilter_Operator_Operator() {}

func (*Filter_Operator_NotExists_) isFilter_Operator_Operator() {}

// == is a special operator that is used to represent an equality filter in a MongoDB
type Filter_Operator_Equal struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Value *Value `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
}

func (x *Filter_Operator_Equal) Reset() {
	*x = Filter_Operator_Equal{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[21]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_Equal) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_Equal) ProtoMessage() {}

func (x *Filter_Operator_Equal) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[21]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_Equal.ProtoReflect.Descriptor instead.
func (*Filter_Operator_Equal) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 0}
}

func (x *Filter_Operator_Equal) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Filter_Operator_Equal) GetValue() *Value {
	if x != nil {
		return x.Value
	}
	return nil
}

// != is a special operator that is used to represent an inequality filter in a MongoDB
type Filter_Operator_NotEqual struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Value *Value `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
}

func (x *Filter_Operator_NotEqual) Reset() {
	*x = Filter_Operator_NotEqual{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[22]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_NotEqual) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_NotEqual) ProtoMessage() {}

func (x *Filter_Operator_NotEqual) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[22]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_NotEqual.ProtoReflect.Descriptor instead.
func (*Filter_Operator_NotEqual) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 1}
}

func (x *Filter_Operator_NotEqual) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Filter_Operator_NotEqual) GetValue() *Value {
	if x != nil {
		return x.Value
	}
	return nil
}

// < is a special operator that is used to represent a less than filter in a MongoDB
type Filter_Operator_Less struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Value *Value `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
}

func (x *Filter_Operator_Less) Reset() {
	*x = Filter_Operator_Less{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[23]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_Less) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_Less) ProtoMessage() {}

func (x *Filter_Operator_Less) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[23]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_Less.ProtoReflect.Descriptor instead.
func (*Filter_Operator_Less) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 2}
}

func (x *Filter_Operator_Less) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Filter_Operator_Less) GetValue() *Value {
	if x != nil {
		return x.Value
	}
	return nil
}

// <= is a special operator that is used to represent a less than or equal to filter in a MongoDB
type Filter_Operator_LessEqual struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Value *Value `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
}

func (x *Filter_Operator_LessEqual) Reset() {
	*x = Filter_Operator_LessEqual{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[24]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_LessEqual) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_LessEqual) ProtoMessage() {}

func (x *Filter_Operator_LessEqual) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[24]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_LessEqual.ProtoReflect.Descriptor instead.
func (*Filter_Operator_LessEqual) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 3}
}

func (x *Filter_Operator_LessEqual) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Filter_Operator_LessEqual) GetValue() *Value {
	if x != nil {
		return x.Value
	}
	return nil
}

// > is a special operator that is used to represent a greater than filter in a MongoDB
type Filter_Operator_Greater struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Value *Value `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
}

func (x *Filter_Operator_Greater) Reset() {
	*x = Filter_Operator_Greater{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[25]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_Greater) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_Greater) ProtoMessage() {}

func (x *Filter_Operator_Greater) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[25]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_Greater.ProtoReflect.Descriptor instead.
func (*Filter_Operator_Greater) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 4}
}

func (x *Filter_Operator_Greater) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Filter_Operator_Greater) GetValue() *Value {
	if x != nil {
		return x.Value
	}
	return nil
}

// >= is a special operator that is used to represent a greater than or equal to filter in a MongoDB
type Filter_Operator_GreaterEqual struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Value *Value `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
}

func (x *Filter_Operator_GreaterEqual) Reset() {
	*x = Filter_Operator_GreaterEqual{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[26]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_GreaterEqual) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_GreaterEqual) ProtoMessage() {}

func (x *Filter_Operator_GreaterEqual) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[26]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_GreaterEqual.ProtoReflect.Descriptor instead.
func (*Filter_Operator_GreaterEqual) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 5}
}

func (x *Filter_Operator_GreaterEqual) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Filter_Operator_GreaterEqual) GetValue() *Value {
	if x != nil {
		return x.Value
	}
	return nil
}

// in is a special operator that is used to represent an in filter in a MongoDB
type Filter_Operator_In struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field  string   `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Values []*Value `protobuf:"bytes,2,rep,name=values,proto3" json:"values,omitempty"`
}

func (x *Filter_Operator_In) Reset() {
	*x = Filter_Operator_In{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[27]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_In) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_In) ProtoMessage() {}

func (x *Filter_Operator_In) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[27]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_In.ProtoReflect.Descriptor instead.
func (*Filter_Operator_In) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 6}
}

func (x *Filter_Operator_In) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Filter_Operator_In) GetValues() []*Value {
	if x != nil {
		return x.Values
	}
	return nil
}

// nin is a special operator that is used to represent a not in filter in a MongoDB
type Filter_Operator_NotIn struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field  string   `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
	Values []*Value `protobuf:"bytes,2,rep,name=values,proto3" json:"values,omitempty"`
}

func (x *Filter_Operator_NotIn) Reset() {
	*x = Filter_Operator_NotIn{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[28]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_NotIn) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_NotIn) ProtoMessage() {}

func (x *Filter_Operator_NotIn) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[28]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_NotIn.ProtoReflect.Descriptor instead.
func (*Filter_Operator_NotIn) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 7}
}

func (x *Filter_Operator_NotIn) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

func (x *Filter_Operator_NotIn) GetValues() []*Value {
	if x != nil {
		return x.Values
	}
	return nil
}

// Exists is a special operator that is used to represent an exists filter in a MongoDB
type Filter_Operator_Exists struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
}

func (x *Filter_Operator_Exists) Reset() {
	*x = Filter_Operator_Exists{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[29]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_Exists) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_Exists) ProtoMessage() {}

func (x *Filter_Operator_Exists) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[29]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_Exists.ProtoReflect.Descriptor instead.
func (*Filter_Operator_Exists) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 8}
}

func (x *Filter_Operator_Exists) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

// Not exists is a special operator that is used to represent a not exists filter in a MongoDB
type Filter_Operator_NotExists struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Field string `protobuf:"bytes,1,opt,name=field,proto3" json:"field,omitempty"`
}

func (x *Filter_Operator_NotExists) Reset() {
	*x = Filter_Operator_NotExists{}
	if protoimpl.UnsafeEnabled {
		mi := &file_mongorpc_proto_msgTypes[30]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Filter_Operator_NotExists) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter_Operator_NotExists) ProtoMessage() {}

func (x *Filter_Operator_NotExists) ProtoReflect() protoreflect.Message {
	mi := &file_mongorpc_proto_msgTypes[30]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter_Operator_NotExists.ProtoReflect.Descriptor instead.
func (*Filter_Operator_NotExists) Descriptor() ([]byte, []int) {
	return file_mongorpc_proto_rawDescGZIP(), []int{6, 0, 9}
}

func (x *Filter_Operator_NotExists) GetField() string {
	if x != nil {
		return x.Field
	}
	return ""
}

var File_mongorpc_proto protoreflect.FileDescriptor

var file_mongorpc_proto_rawDesc = []byte{
	,
}

var (
	file_mongorpc_proto_rawDescOnce sync.Once
	file_mongorpc_proto_rawDescData = file_mongorpc_proto_rawDesc
)

func file_mongorpc_proto_rawDescGZIP() []byte {
	file_mongorpc_proto_rawDescOnce.Do(func() {
		file_mongorpc_proto_rawDescData = protoimpl.X.CompressGZIP(file_mongorpc_proto_rawDescData)
	})
	return file_mongorpc_proto_rawDescData
}

var file_mongorpc_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_mongorpc_proto_msgTypes = make([]protoimpl.MessageInfo, 31)
var file_mongorpc_proto_goTypes = []interface{}{
	(NullValue)(0),                       // 0: mongorpc.NullValue
	(*ArrayValue)(nil),                   // 1: mongorpc.ArrayValue
	(*MapValue)(nil),                     // 2: mongorpc.MapValue
	(*Timestamp)(nil),                    // 3: mongorpc.Timestamp
	(*ObjectID)(nil),                     // 4: mongorpc.ObjectID
	(*Value)(nil),                        // 5: mongorpc.Value
	(*Sort)(nil),                         // 6: mongorpc.Sort
	(*Filter)(nil),                       // 7: mongorpc.Filter
	(*ListCollectionsRequest)(nil),       // 8: mongorpc.ListCollectionsRequest
	(*ListCollectionsResponse)(nil),      // 9: mongorpc.ListCollectionsResponse
	(*GetDocumentRequest)(nil),           // 10: mongorpc.GetDocumentRequest
	(*GetDocumentResponse)(nil),          // 11: mongorpc.GetDocumentResponse
	(*ListDocumentsRequest)(nil),         // 12: mongorpc.ListDocumentsRequest
	(*ListDocumentsResponse)(nil),        // 13: mongorpc.ListDocumentsResponse
	(*CreateDocumentRequest)(nil),        // 14: mongorpc.CreateDocumentRequest
	(*CreateDocumentResponse)(nil),       // 15: mongorpc.CreateDocumentResponse
	(*UpdateDocumentRequest)(nil),        // 16: mongorpc.UpdateDocumentRequest
	(*UpdateDocumentResponse)(nil),       // 17: mongorpc.UpdateDocumentResponse
	(*DeleteDocumentRequest)(nil),        // 18: mongorpc.DeleteDocumentRequest
	(*DeleteDocumentResponse)(nil),       // 19: mongorpc.DeleteDocumentResponse
	nil,                                  // 20: mongorpc.MapValue.FieldsEntry
	(*Filter_Operator)(nil),              // 21: mongorpc.Filter.Operator
	(*Filter_Operator_Equal)(nil),        // 22: mongorpc.Filter.Operator.Equal
	(*Filter_Operator_NotEqual)(nil),     // 23: mongorpc.Filter.Operator.NotEqual
	(*Filter_Operator_Less)(nil),         // 24: mongorpc.Filter.Operator.Less
	(*Filter_Operator_LessEqual)(nil),    // 25: mongorpc.Filter.Operator.LessEqual
	(*Filter_Operator_Greater)(nil),      // 26: mongorpc.Filter.Operator.Greater
	(*Filter_Operator_GreaterEqual)(nil), // 27: mongorpc.Filter.Operator.GreaterEqual
	(*Filter_Operator_In)(nil),           // 28: mongorpc.Filter.Operator.In
	(*Filter_Operator_NotIn)(nil),        // 29: mongorpc.Filter.Operator.NotIn
	(*Filter_Operator_Exists)(nil),       // 30: mongorpc.Filter.Operator.Exists
	(*Filter_Operator_NotExists)(nil),    // 31: mongorpc.Filter.Operator.NotExists
}
var file_mongorpc_proto_depIdxs = []int32{
	5,  // 0: mongorpc.ArrayValue.values:type_name -> mongorpc.Value
	20, // 1: mongorpc.MapValue.fields:type_name -> mongorpc.MapValue.FieldsEntry
	2,  // 2: mongorpc.Value.map_value:type_name -> mongorpc.MapValue
	1,  // 3: mongorpc.Value.array_value:type_name -> mongorpc.ArrayValue
	3,  // 4: mongorpc.Value.date_value:type_name -> mongorpc.Timestamp
	0,  // 5: mongorpc.Value.null_value:type_name -> mongorpc.NullValue
	4,  // 6: mongorpc.Value.object_id_value:type_name -> mongorpc.ObjectID
	1,  // 7: mongorpc.ListCollectionsResponse.collections:type_name -> mongorpc.ArrayValue
	5,  // 8: mongorpc.GetDocumentResponse.document:type_name -> mongorpc.Value
	7,  // 9: mongorpc.ListDocumentsRequest.filter:type_name -> mongorpc.Filter
	6,  // 10: mongorpc.ListDocumentsRequest.sort:type_name -> mongorpc.Sort
	1,  // 11: mongorpc.ListDocumentsResponse.documents:type_name -> mongorpc.ArrayValue
	5,  // 12: mongorpc.CreateDocumentRequest.document:type_name -> mongorpc.Value
	5,  // 13: mongorpc.UpdateDocumentRequest.document:type_name -> mongorpc.Value
	5,  // 14: mongorpc.UpdateDocumentResponse.upserted_id:type_name -> mongorpc.Value
	5,  // 15: mongorpc.MapValue.FieldsEntry.value:type_name -> mongorpc.Value
	22, // 16: mongorpc.Filter.Operator.equal:type_name -> mongorpc.Filter.Operator.Equal
	23, // 17: mongorpc.Filter.Operator.not_equal:type_name -> mongorpc.Filter.Operator.NotEqual
	24, // 18: mongorpc.Filter.Operator.less:type_name -> mongorpc.Filter.Operator.Less
	25, // 19: mongorpc.Filter.Operator.less_equal:type_name -> mongorpc.Filter.Operator.LessEqual
	26, // 20: mongorpc.Filter.Operator.greater:type_name -> mongorpc.Filter.Operator.Greater
	27, // 21: mongorpc.Filter.Operator.greater_equal:type_name -> mongorpc.Filter.Operator.GreaterEqual
	28, // 22: mongorpc.Filter.Operator.in:type_name -> mongorpc.Filter.Operator.In
	29, // 23: mongorpc.Filter.Operator.not_in:type_name -> mongorpc.Filter.Operator.NotIn
	30, // 24: mongorpc.Filter.Operator.exists:type_name -> mongorpc.Filter.Operator.Exists
	31, // 25: mongorpc.Filter.Operator.not_exists:type_name -> mongorpc.Filter.Operator.NotExists
	5,  // 26: mongorpc.Filter.Operator.Equal.value:type_name -> mongorpc.Value
	5,  // 27: mongorpc.Filter.Operator.NotEqual.value:type_name -> mongorpc.Value
	5,  // 28: mongorpc.Filter.Operator.Less.value:type_name -> mongorpc.Value
	5,  // 29: mongorpc.Filter.Operator.LessEqual.value:type_name -> mongorpc.Value
	5,  // 30: mongorpc.Filter.Operator.Greater.value:type_name -> mongorpc.Value
	5,  // 31: mongorpc.Filter.Operator.GreaterEqual.value:type_name -> mongorpc.Value
	5,  // 32: mongorpc.Filter.Operator.In.values:type_name -> mongorpc.Value
	5,  // 33: mongorpc.Filter.Operator.NotIn.values:type_name -> mongorpc.Value
	8,  // 34: mongorpc.MongoRPC.ListCollections:input_type -> mongorpc.ListCollectionsRequest
	10, // 35: mongorpc.MongoRPC.GetDocument:input_type -> mongorpc.GetDocumentRequest
	12, // 36: mongorpc.MongoRPC.ListDocuments:input_type -> mongorpc.ListDocumentsRequest
	14, // 37: mongorpc.MongoRPC.CreateDocument:input_type -> mongorpc.CreateDocumentRequest
	16, // 38: mongorpc.MongoRPC.UpdateDocument:input_type -> mongorpc.UpdateDocumentRequest
	18, // 39: mongorpc.MongoRPC.DeleteDocument:input_type -> mongorpc.DeleteDocumentRequest
	9,  // 40: mongorpc.MongoRPC.ListCollections:output_type -> mongorpc.ListCollectionsResponse
	11, // 41: mongorpc.MongoRPC.GetDocument:output_type -> mongorpc.GetDocumentResponse
	13, // 42: mongorpc.MongoRPC.ListDocuments:output_type -> mongorpc.ListDocumentsResponse
	15, // 43: mongorpc.MongoRPC.CreateDocument:output_type -> mongorpc.CreateDocumentResponse
	17, // 44: mongorpc.MongoRPC.UpdateDocument:output_type -> mongorpc.UpdateDocumentResponse
	19, // 45: mongorpc.MongoRPC.DeleteDocument:output_type -> mongorpc.DeleteDocumentResponse
	40, // [40:46] is the sub-list for method output_type
	34, // [34:40] is the sub-list for method input_type
	34, // [34:34] is the sub-list for extension type_name
	34, // [34:34] is the sub-list for extension extendee
	0,  // [0:34] is the sub-list for field type_name
}

func init() { file_mongorpc_proto_init() }
func file_mongorpc_proto_init() {
	if File_mongorpc_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_mongorpc_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ArrayValue); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*MapValue); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Timestamp); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ObjectID); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Value); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Sort); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListCollectionsRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListCollectionsResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[9].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetDocumentRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[10].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*GetDocumentResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[11].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListDocumentsRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[12].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListDocumentsResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[13].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateDocumentRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[14].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateDocumentResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[15].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateDocumentRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[16].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateDocumentResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[17].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteDocumentRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[18].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteDocumentResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[20].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[21].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_Equal); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[22].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_NotEqual); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[23].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_Less); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[24].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_LessEqual); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[25].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_Greater); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[26].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_GreaterEqual); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[27].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_In); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[28].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_NotIn); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[29].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_Exists); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_mongorpc_proto_msgTypes[30].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Filter_Operator_NotExists); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	file_mongorpc_proto_msgTypes[4].OneofWrappers = []interface{}{
		(*Value_DoubleValue)(nil),
		(*Value_StringValue)(nil),
		(*Value_MapValue)(nil),
		(*Value_ArrayValue)(nil),
		(*Value_BytesValue)(nil),
		(*Value_BoolValue)(nil),
		(*Value_DateValue)(nil),
		(*Value_NullValue)(nil),
		(*Value_IntegerValue)(nil),
		(*Value_ObjectIdValue)(nil),
	}
	file_mongorpc_proto_msgTypes[20].OneofWrappers = []interface{}{
		(*Filter_Operator_Equal_)(nil),
		(*Filter_Operator_NotEqual_)(nil),
		(*Filter_Operator_Less_)(nil),
		(*Filter_Operator_LessEqual_)(nil),
		(*Filter_Operator_Greater_)(nil),
		(*Filter_Operator_GreaterEqual_)(nil),
		(*Filter_Operator_In_)(nil),
		(*Filter_Operator_NotIn_)(nil),
		(*Filter_Operator_Exists_)(nil),
		(*Filter_Operator_NotExists_)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_mongorpc_proto_rawDesc,
			NumEnums:      1,
			NumMessages:   31,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_mongorpc_proto_goTypes,
		DependencyIndexes: file_mongorpc_proto_depIdxs,
		EnumInfos:         file_mongorpc_proto_enumTypes,
		MessageInfos:      file_mongorpc_proto_msgTypes,
	}.Build()
	File_mongorpc_proto = out.File
	file_mongorpc_proto_rawDesc = nil
	file_mongorpc_proto_goTypes = nil
	file_mongorpc_proto_depIdxs = nil
}
